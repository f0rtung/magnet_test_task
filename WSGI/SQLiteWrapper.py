...
class SQLiteWrapper:
    ...
    def get_all_cities_by_reg_id(self, region_id):
    ...
    def remove_comment(self, comment_id):
        self.delete_query("DELETE FROM CommentInfo WHERE CommentID = ?", (comment_id,))

    def insert_comment(self, comment_data):
        f_name = comment_data["fName"]
        s_name = comment_data["sName"]
        m_name = comment_data.get("mName", "")
        reg_id = int(comment_data["region"])
        city_id = int(comment_data["city"])
        email = comment_data["email"]
        phone = comment_data["phone"]
        comment = comment_data["comment"]
        reg_city_relation = self.get_reg_city_relation(reg_id, city_id)[0][0]
        self.insert_query("INSERT INTO CommentInfo "
                          "(LastName, FirstName, MiddleName, RCRelation, Phone, Email, Comment) "
                          "VALUES(?, ?, ?, ?, ?, ?, ?)",
                          (s_name, f_name, m_name, reg_city_relation, phone, email, comment))

    def get_reg_city_relation(self, reg_id, city_id):
        return self.select_query("SELECT RelationID "
                                 "FROM RegionCityRelations "
                                 "WHERE RegionID = ? AND CityID = ?", (reg_id, city_id))

    def get_all_comments(self):
        return self.select_query("SELECT LastName, FirstName, MiddleName, "
    ...
    def get_region_where_comments_more_than(self, comm_count):
        return self.select_query("SELECT Regions.RegionID, Regions.RegionName, count(CommentID) as commCount "
                                 "FROM CommentInfo comInf "
                                 "JOIN RegionCityRelations rcRel ON comInf.RCRelation = rcRel.RelationID "
                                 "JOIN Regions ON rcRel.RegionID = Regions.RegionID "
                                 "GROUP BY Regions.RegionID "
                                 "HAVING commCount > ?", (comm_count,))

    def get_cities_stat_by_region_id(self, region_id):
        return self.select_query("SELECT Cities.CityName, count(CommentID) as commCount "
                                 "FROM CommentInfo comInf "
                                 "JOIN RegionCityRelations rcRel ON comInf.RCRelation = rcRel.RelationID "
                                 "JOIN Cities ON rcRel.CityID = Cities.CityID "
                                 "JOIN Regions ON rcRel.RegionID = Regions.RegionID "
                                 "GROUP BY Cities.CityID "
                                 "HAVING Regions.RegionID = ?", (region_id,))
